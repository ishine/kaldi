...
# b7 is based on b2, but we use PerElementScale component instead of NaturalGradientPerElementScale component 
#System                   vfsmn_b7    vfsmn_b2
set -e
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/dfsmn_500h_d1_4epoch  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
...
frames_per_eg=150
remove_egs=false
common_egs_dir=exp/chain/dfsmn_500h_d1_4epoch/egs
xent_regularize=0.1
...
if [ $stage -le 12 ]; then
  ...
  relu-batchnorm-layer name=prefinal-chain input=dfsmn9_projection $opts dim=1536
  output-layer name=output include-log-softmax=false dim=$num_targets $output_opts

  relu-batchnorm-layer name=prefinal-xent input=dfsmn9_projection $opts dim=1536
...
!
decode_suff=online
graph_dir=exp/chain/dfsmn_500h_d1_4epoch/graph_online
if [ $stage -le 15 ]; then
  iter_opts=
