...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_20M_500h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=/public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/exp/chain/lstm_6j_16k_500h_ld5/egs
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmr-layer name=fastlstm1 input=Append(-2,-1,0,1,2) cell-dim=750 recurrent-projection-dim=240 delay=-3
  fast-lstmr-layer name=fastlstm2 cell-dim=750 recurrent-projection-dim=240 delay=-3
  fast-lstmr-layer name=fastlstm3 cell-dim=750 recurrent-projection-dim=240 delay=-3
