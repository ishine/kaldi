...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_online_sogoufeat_7000h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmr-layer name=fastlstm1 input=Append(-2,-1,0,1,2) cell-dim=3072 recurrent-projection-dim=512 delay=-3
  fast-lstmr-layer name=fastlstm2 cell-dim=2560 recurrent-projection-dim=512 delay=-3
  fast-lstmr-layer name=fastlstm3 cell-dim=2560 recurrent-projection-dim=512 delay=-3
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --trainer.optimization.final-effective-lrate 0.00018 \
