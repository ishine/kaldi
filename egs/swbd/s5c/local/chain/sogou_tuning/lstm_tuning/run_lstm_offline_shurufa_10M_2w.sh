...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_offline_shurufa_10M_2w # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmr-layer name=fastlstm1 input=Append(-2,-1,0,1,2) cell-dim=512 recurrent-projection-dim=160 delay=-3
  fast-lstmr-layer name=fastlstm2 cell-dim=512 recurrent-projection-dim=160 delay=-3
  fast-lstmr-layer name=fastlstm3 cell-dim=512 recurrent-projection-dim=160 delay=-3
