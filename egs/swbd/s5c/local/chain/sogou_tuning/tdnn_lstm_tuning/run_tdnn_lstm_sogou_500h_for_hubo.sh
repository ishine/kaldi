...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_sogoufeat_500hours_noL2_noCE # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  output-layer name=output-xent input=fastlstm3 output-delay=$label_delay dim=$num_targets max-change=1.5
