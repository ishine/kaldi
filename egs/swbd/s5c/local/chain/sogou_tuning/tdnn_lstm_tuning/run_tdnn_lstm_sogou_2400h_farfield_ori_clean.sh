...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_2400h_farfield_ori_clean_LFR_noLD # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=/public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/exp/chain/tdnn_lstm_1c_2400h_farfield_ori_clean_LFR_noLD/egs
...
dir=${dir}$suffix
train_set=train_sogou_fbank_2400h_farfield_ori_clean
...
if [ $stage -le 12 ]; then
  ...
  output-layer name=output input=fastlstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=0.75
