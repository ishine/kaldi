...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_sogoufeat_7300hours_delay18 # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn1 input=Append(-1,0,1) dim=1024
  relu-renorm-layer name=tdnn2 dim=1024
  relu-renorm-layer name=tdnn3 dim=1024
