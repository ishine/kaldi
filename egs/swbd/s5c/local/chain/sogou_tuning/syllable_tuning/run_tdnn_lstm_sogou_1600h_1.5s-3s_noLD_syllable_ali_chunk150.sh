...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_sogoufeat_1600hours_1.5s-3s_syllable_chunk150_noLD # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
dir=${dir}$suffix
train_set=train_sogou_fbank_1.5s-3s_1600h
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn2 input=Append(-3,0,3) dim=768
  relu-renorm-layer name=tdnn3 input=Append(-3,0,3) dim=768

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  fast-lstmr-layer name=fastlstm1 cell-dim=1024 recurrent-projection-dim=384 delay=-3
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=768
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=768
  fast-lstmr-layer name=fastlstm2 cell-dim=1024 recurrent-projection-dim=384 delay=-3
  relu-renorm-layer name=tdnn6 input=Append(-3,0,3) dim=768
  relu-renorm-layer name=tdnn7 input=Append(-3,0,3) dim=768
...
if [ $stage -le 13 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir exp/tri3b_ali_syllable_length_1600h_1.5s-3s \
    --dir $dir  || exit 1;
fi
<<!
if [ $stage -le 14 ]; then
