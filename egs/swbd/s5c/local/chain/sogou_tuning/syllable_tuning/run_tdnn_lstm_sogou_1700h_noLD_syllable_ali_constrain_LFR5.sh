...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_sogoufeat_1700hours_noLD_syllable_ali_constrain_5s_2gram_denfst_L5R5_LFR5 # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn2 input=Append(-5,0,5) dim=768
  relu-renorm-layer name=tdnn3 input=Append(-5,0,5) dim=768

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  fast-lstmr-layer name=fastlstm1 cell-dim=1024 recurrent-projection-dim=384 delay=-5
  relu-renorm-layer name=tdnn4 input=Append(-5,0,5) dim=1024
  relu-renorm-layer name=tdnn5 input=Append(-5,0,5) dim=1024
  fast-lstmr-layer name=fastlstm2 cell-dim=1024 recurrent-projection-dim=384 delay=-5
  relu-renorm-layer name=tdnn6 input=Append(-5,0,5) dim=1024
  relu-renorm-layer name=tdnn7 input=Append(-5,0,5) dim=1024
  fast-lstmr-layer name=fastlstm3 cell-dim=1024 recurrent-projection-dim=384 delay=-5
