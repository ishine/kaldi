...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/3tdnn_3blstm_1700h_5s_syllable_ali_constrain_2gram_L55R55_chunk150_nnet3Lats_LFR4 # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=1200
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn1 input=Append(-3,-2,-1,0,1,2,3) dim=625
  relu-renorm-layer name=tdnn2 input=Append(-4,0,4) dim=625 
  relu-renorm-layer name=tdnn3 input=Append(-4,0,4) dim=625 

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  fast-lstmr-layer name=blstm1-forward input=tdnn3 cell-dim=1024 recurrent-projection-dim=192 delay=-4
  fast-lstmr-layer name=blstm1-backward input=tdnn3 cell-dim=1024 recurrent-projection-dim=192 delay=4
  fast-lstmr-layer name=blstm2-forward input=Append(blstm1-forward, blstm1-backward) cell-dim=768 recurrent-projection-dim=192 delay=-4
  fast-lstmr-layer name=blstm2-backward input=Append(blstm1-forward, blstm1-backward) cell-dim=768 recurrent-projection-dim=192 delay=4
  fast-lstmr-layer name=blstm3-forward input=Append(blstm2-forward, blstm2-backward) cell-dim=768 recurrent-projection-dim=192 delay=-4
  fast-lstmr-layer name=blstm3-backward input=Append(blstm2-forward, blstm2-backward) cell-dim=768 recurrent-projection-dim=192 delay=4
