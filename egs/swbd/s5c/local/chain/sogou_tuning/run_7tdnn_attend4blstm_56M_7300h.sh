...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/7tdnn_attend4blstm_56M_7300h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  no-op-component name=noop input=Append(blstm3-forward, blstm3-backward)
  attention-layer name=attention1 input=noop num-heads=1 value-dim=609 key-dim=512 num-left-inputs=15 num-right-inputs=15 time-stride=3
  batchnorm-component name=atten1.batchnorm1 input=Sum(noop, attention1)
  ...
  fast-lstmr-layer name=blstm4-forward input=atten1.batchnorm2 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm4-backward input=atten1.batchnorm2 cell-dim=1280 recurrent-projection-dim=320 delay=3
