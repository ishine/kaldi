...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_2400h_farfield_ori_clean_SimuRIR_isotroNoise_LFR_noLD_LR0.002_TS # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=exp/chain/tdnn_lstm_1c_2400h_farfield_ori_clean_SimuRIR_isotroNoise_LFR_noLD_TS/egs
...
dir=${dir}$suffix
train_set=train_sogou_fbank_2400h_farfield_ori_clean_SimuRIR_isotropic_noise_TS
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn2 input=Append(-3,0,3) dim=768 max-change=0.75
  relu-renorm-layer name=tdnn3 input=Append(-3,0,3) dim=768 max-change=0.75

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  fast-lstmr-layer name=fastlstm1 cell-dim=1280 recurrent-projection-dim=512 delay=-3
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=768 max-change=1.0
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=768 max-change=1.0
  fast-lstmr-layer name=fastlstm2 cell-dim=1280 recurrent-projection-dim=384 delay=-3
  relu-renorm-layer name=tdnn6 input=Append(-3,0,3) dim=768 max-change=1.0
  relu-renorm-layer name=tdnn7 input=Append(-3,0,3) dim=768 max-change=1.0
  fast-lstmr-layer name=fastlstm3 cell-dim=1280 recurrent-projection-dim=384 delay=-3

  ## adding the layers for chain branch
  output-layer name=output input=fastlstm3 output-delay=$label_delay include-log-softmax=true dim=$num_targets max-change=0.75
  ...
  # output-layer name=output-xent input=fastlstm3 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=0.75
...
if [ $stage -le 13 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir /public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/exp/tri3b_2400h_farfield_ori_clean_lats \
    --dir $dir  || exit 1;
fi
<<!
if [ $stage -le 14 ]; then
...
#  for decode_set in not_on_screen_sogou test8000_sogou testIOS_sogou testset_testND_sogou; do
  for decode_set in  not_on_screen_sogou test8000_sogou testIOS_sogou testset_testND_sogou \
       testSmallRoom1hours_sogou testMiddleRoom1hours_sogou testBigRoom1hours_sogou testmvdr_sogou testfarfiled_sogou; do
