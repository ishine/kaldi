...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/lstm_6j_1024_256_sogoufeat_500h_TS_softmax_LR0.003 # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  # input dim=71 name=input-teacher
  ...
  output-layer name=output input=fastlstm3 output-delay=$label_delay include-log-softmax=true dim=$num_targets learning-rate-factor=1 max-change=1.5
  ...
  # similar in the xent and regular final layers.
   output-layer name=output-xent input=fastlstm3 output-delay=$label_delay dim=$num_targets learning-rate-factor=1 max-change=1.5
