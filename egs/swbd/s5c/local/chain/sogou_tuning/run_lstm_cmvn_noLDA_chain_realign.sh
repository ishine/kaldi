...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_chain_realign # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=exp/chain/lstm_6j_chain_realign_ld5/egs
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm1 input=Append(-2,-1,0,1,2) cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3
...
decode_suff=bigG
graph_dir=exp/chain/lstm_6j_noLDA_ld5/graph
if [ $stage -le 15 ]; then
