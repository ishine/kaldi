...
# run_tdnn_lstm_attend_a8.sh is like run_tdnn_lstm_attend_a7.sh but use longer context L10R10
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_attend_a8_500h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  attention-relu-renorm-layer name=attention1 num-heads=15 value-dim=80 key-dim=40 num-left-inputs=10 num-right-inputs=10 time-stride=3
