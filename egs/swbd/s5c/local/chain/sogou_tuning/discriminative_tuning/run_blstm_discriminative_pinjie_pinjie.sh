...
srcdir=exp/chain/9tdnn_4blstm_65M_10wh_adapt_init35000_2epoch_smbr_stage1_smbr_stage2_smbr_stage_smbr_C1_S1_CTS
train_data_dir=data/train_sogou_fbank_11w_sdt_stage4_C1_and_S1_and_stage3_5000h_part1
online_ivector_dir=
...
##    rm ${online_ivector_dir}_fs/ivector_online.scp 2>/dev/null || true
    utils/data/shift_and_combine_feats.sh $frame_subsampling_factor $train_data_dir ${train_data_dir}_fs 
  fi

  train_data_dir=${train_data_dir}_fs

  affix=_fs_C1_S1_CTS_part1
fi
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    num_threads_denlats=5
    subsplit=4 # number of jobs that run per job (but 2 run at a time, so total jobs is 80, giving
    # total slots = 80 * 6 = 480.
    steps/nnet3/make_denlats.sh --cmd "$decode_cmd" --use-gpu true \
...
if [ $stage -le 6 ]; then
  for decode_set in $testsets; do
    steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
      /public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/data/lang_0528 /public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/data/lang_0528_lr \
      /public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/data/${decode_set} \
