...
srcdir=exp/chain/lstm_6j_online_49M_4w5_ld5
...
if [ $stage -le 1 ]; then
  ...
  nj=120 # have a high number of jobs because this could take a while, and we might
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    num_threads_denlats=4
    subsplit=12 # number of jobs that run per job (but 2 run at a time, so total jobs is 80, giving
    # total slots = 80 * 6 = 480.
    steps/nnet3/make_denlats.sh --cmd "$decode_only" --use-gpu true \
