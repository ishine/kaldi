...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/13tdnn_5blstm_skip_61M_7300h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=exp/chain/11tdnn_5blstm_67M_7300h/egs
...
if [ $stage -le 12 ]; then
  ...
  linear-component name=tdnn3 input=Append(-3,0,3) dim=512 max-change=1.0
  ...
  relu-renorm-layer name=tdnn6 input=Append(tdnn3,tdnn5) dim=512 max-change=0.75
  fast-lstmr-layer name=blstm2-forward input=tdnn6 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm2-backward input=tdnn6 cell-dim=1280 recurrent-projection-dim=320 delay=3
  relu-renorm-layer name=tdnn7 input=Append(blstm2-forward, blstm2-backward) dim=1024 max-change=0.75
  linear-component name=tdnn8 input=Append(-3,0,3) dim=512 $linear_opts
  fast-lstmr-layer name=blstm3-forward input=tdnn8 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm3-backward input=tdnn8 cell-dim=1280 recurrent-projection-dim=320 delay=3
  relu-renorm-layer name=tdnn9 input=Append(blstm3-forward, blstm3-backward) dim=1024 max-change=0.75
  linear-component name=tdnn10 input=Append(-3,0,3) dim=512 $linear_opts
  fast-lstmr-layer name=blstm4-forward input=tdnn10 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm4-backward input=tdnn10 cell-dim=1280 recurrent-projection-dim=320 delay=3
  relu-renorm-layer name=tdnn11 input=Append(blstm4-forward, blstm4-backward) dim=1024 max-change=0.75
  linear-component name=tdnn12 input=Append(-3,0,3) dim=512 $linear_opts
  relu-renorm-layer name=tdnn13 input=Append(tdnn12,tdnn10,tdnn8) dim=512 max-change=0.75
  fast-lstmr-layer name=blstm5-forward input=tdnn13 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm5-backward input=tdnn13 cell-dim=1280 recurrent-projection-dim=320 delay=3

  ## adding the layers for chain branch
  output-layer name=output input=Append(blstm5-forward, blstm5-backward) output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.0
  ...
  output-layer name=output-xent input=Append(blstm5-forward, blstm5-backward) output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.0
...
#  for decode_set in not_on_screen_sogou test8000_sogou testIOS_sogou testset_testND_sogou; do
  for decode_set in not_on_screen_sogou; do
