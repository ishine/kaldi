...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/11tdnn_5blstm_67M_7300h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn10 input=Append(blstm4-forward, blstm4-backward) dim=1024 max-change=0.75
  linear-component name=tdnn11 input=Append(-3,0,3) dim=512 $linear_opts
  fast-lstmr-layer name=blstm5-forward input=tdnn11 cell-dim=1280 recurrent-projection-dim=320 delay=-3
  fast-lstmr-layer name=blstm5-backward input=tdnn11 cell-dim=1280 recurrent-projection-dim=320 delay=3
...
#  for decode_set in not_on_screen_sogou test8000_sogou testIOS_sogou testset_testND_sogou; do
  for decode_set in test8000_sogou testIOS_sogou testset_testND_sogou; do
