...
get_egs_stage=5
speed_perturb=false
dir=/public/speech/wangzhichao/kaldi/kaldi-wzc/egs/sogou/s5c/exp/chain/9tdnn_4blstm_65M_10wh # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  linear_opts="orthonormal-constraint=-1.0 max-change=1.0"
  ...
  relu-renorm-layer name=tdnn2 input=Append(-3,0,3) dim=1280 max-change=1.2
  relu-renorm-layer name=tdnn3 input=Append(-3,0,3) dim=1024 max-change=1.2

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  fast-lstmr-layer name=blstm1-forward input=tdnn3 cell-dim=1280 recurrent-projection-dim=384 delay=-3
  fast-lstmr-layer name=blstm1-backward input=tdnn3 cell-dim=1280 recurrent-projection-dim=384 delay=3
  relu-renorm-layer name=tdnn4 input=Append(blstm1-forward, blstm1-backward) dim=1536 max-change=0.75
  linear-component name=tdnn5 input=Append(-3,0,3) dim=512 $linear_opts
  fast-lstmr-layer name=blstm2-forward input=tdnn5 cell-dim=1280 recurrent-projection-dim=384 delay=-3
  fast-lstmr-layer name=blstm2-backward input=tdnn5 cell-dim=1280 recurrent-projection-dim=384 delay=3
  relu-renorm-layer name=tdnn6 input=Append(blstm2-forward, blstm2-backward) dim=1536 max-change=0.75
