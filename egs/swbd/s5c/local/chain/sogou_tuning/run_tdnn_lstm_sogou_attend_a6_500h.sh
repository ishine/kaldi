...
# run_tdnn_lstm_attend_a6.sh is like run_tdnn_lstm_attend_a1.sh but use longer context on the right
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm_1c_attend_a6_500h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  attention-relu-renorm-layer name=attention1 num-heads=15 value-dim=80 key-dim=40 num-left-inputs=5 num-right-inputs=5 time-stride=3
