...
# run_tdnn_attend_a5.sh is like run_tdnn_lstm_attend_a5.sh but replace all lstm layers with attentions layers 
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_attend_500h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn4 dim=625
  ...
  attention-relu-renorm-layer name=attention5 num-heads=15 value-dim=80 key-dim=40 num-left-inputs=5 num-right-inputs=2 time-stride=3
  ## adding the layers for chain branch
  output-layer name=output input=attention5 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=attention5 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
