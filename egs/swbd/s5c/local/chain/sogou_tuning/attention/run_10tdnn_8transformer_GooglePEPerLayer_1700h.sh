...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/10tdnn_8transformerBlock_fullAttention_GooglePEPerLayer_chunk200_1700h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  selfattention-layer name=attention7 num-heads=1 value-dim=625 key-dim=512 num-left-inputs=0 num-right-inputs=0 time-stride=3
  ...
  selfattention-layer name=attention8 num-heads=1 value-dim=625 key-dim=512 num-left-inputs=0 num-right-inputs=0 time-stride=3
...
if [ $stage -le 15 ]; then
  ...
  for decode_set in not_on_screen_sogou test8000_sogou testIOS_sogou testset_testND_sogou; do
       steps/nnet3/decode_sogou.sh --use_gpu false --acwt 1.0 --post-decode-acwt 10.0 \
