...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/3tdnn_3AttendBatchnorm_1700h_18M # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=exp/chain/3tdnn_3AttendBatchnorm_1700h/egs
...
if [ $stage -le 12 ]; then
  ...
  attention-relu-batchnorm-layer name=attention1 num-heads=1 value-dim=2048 key-dim=1024 num-left-inputs=10 num-right-inputs=10 time-stride=3
  attention-relu-batchnorm-layer name=attention2 num-heads=1 value-dim=2048 key-dim=1024 num-left-inputs=10 num-right-inputs=10 time-stride=3
  attention-relu-batchnorm-layer name=attention3 num-heads=1 value-dim=2048 key-dim=1024 num-left-inputs=10 num-right-inputs=10 time-stride=3

  ## adding the layers for chain branch
  output-layer name=output input=attention3 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.0
  ...
  output-layer name=output-xent input=attention3 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.0
