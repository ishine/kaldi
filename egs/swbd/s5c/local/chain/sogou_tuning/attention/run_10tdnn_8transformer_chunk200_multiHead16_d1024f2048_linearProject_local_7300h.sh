...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/10tdnn_8transformerBlock_chunk200_multiHead16_d1024f2048_linearProject_local_7300h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  attention-layer name=attention9 num-heads=16 value-dim=33 key-dim=64 num-left-inputs=15 num-right-inputs=15 time-stride=3 max-change=1.0
  batchnorm-component name=atten9.batchnorm1 input=Sum(atten8.batchnorm2, attention9)
