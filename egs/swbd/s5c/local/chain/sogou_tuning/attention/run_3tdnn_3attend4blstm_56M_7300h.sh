...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/3tdnn_3attend4blstm_7300h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  no-op-component name=noop1 input=Append(blstm1-forward, blstm1-backward)
  attention-layer name=attention1 input=noop1 num-heads=1 value-dim=619 key-dim=512 num-left-inputs=10 num-right-inputs=10 time-stride=3
  batchnorm-component name=atten1.batchnorm1 input=Sum(noop1, attention1)
  relu-layer name=atten1.feedforward1 dim=2048                           
  affine-layer name=atten1.feedforward2 dim=640
  batchnorm-component name=atten1.batchnorm2 input=Sum(atten1.batchnorm1, atten1.feedforward2)
  fast-lstmr-layer name=blstm2-forward input=atten1.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=-3
  fast-lstmr-layer name=blstm2-backward input=atten1.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=3
  no-op-component name=noop2 input=Append(blstm2-forward, blstm2-backward)
  attention-layer name=attention2 input=noop2 num-heads=1 value-dim=491 key-dim=512 num-left-inputs=10 num-right-inputs=10 time-stride=3
  batchnorm-component name=atten2.batchnorm1 input=Sum(noop2, attention2)
  ...
  fast-lstmr-layer name=blstm3-forward input=atten2.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=-3
  fast-lstmr-layer name=blstm3-backward input=atten2.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=3
  no-op-component name=noop3 input=Append(blstm3-forward, blstm3-backward)
  attention-layer name=attention3 input=noop3 num-heads=1 value-dim=481 key-dim=512 num-left-inputs=15 num-right-inputs=15 time-stride=3
  batchnorm-component name=atten3.batchnorm1 input=Sum(noop3, attention3)
  ...
  fast-lstmr-layer name=blstm4-forward input=atten3.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=-3
  fast-lstmr-layer name=blstm4-backward input=atten3.batchnorm2 cell-dim=1280 recurrent-projection-dim=256 delay=3
