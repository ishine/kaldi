...
# b is based on a, but with 5 hidden layer
#System                   vfsmn_a3    vfsmn_a4
set -e
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/dnn_500h_b  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
...
frames_per_eg=150
remove_egs=false
common_egs_dir=exp/chain/dnn_500h_a/egs
xent_regularize=0.1
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn1 input=Append(-1,0,1) dim=625
  relu-renorm-layer name=tdnn2 dim=625
  relu-renorm-layer name=tdnn3 dim=625
  relu-renorm-layer name=tdnn4 dim=625
  ## adding the layers for chain branch
  relu-renorm-layer name=prefinal-chain input=tdnn4 dim=625 target-rms=0.5
  ...
  relu-renorm-layer name=prefinal-xent input=tdnn4 dim=625 target-rms=0.5
