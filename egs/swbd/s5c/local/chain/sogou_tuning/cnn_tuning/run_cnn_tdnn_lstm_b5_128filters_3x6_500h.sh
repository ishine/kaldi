...
# run_cnn_tdnn_lstm_b5_128filters_3x6_500h.sh is like run_cnn_tdnn_lstm_b2_128filters_500h.sh but using different kernel size (3*6 vs 3*9)
# kernel size: 3*6
...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/cnn_tdnn_lstm_b5_128filters_3x6_500h # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  conv-relu-renorm-layer name=cnn1 height-in=71 height-out=23 height-subsample-out=3 time-offsets=-1,0,1 height-offsets=0,1,2,3,4,5 num-filters-out=128 learning-rate-factor=0.333 max-change=0.5
