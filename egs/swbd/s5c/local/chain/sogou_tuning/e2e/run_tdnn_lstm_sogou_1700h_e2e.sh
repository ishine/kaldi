...
lang=data/lang_0528_syllable
treedir=exp/chain/tri5_7000houres_mono_tree_syllable
dir=exp/chain/tdnn_lstm_1700h_syllabel_e2e_2gram_denfst
...
if [ $stage -le 2 ]; then
  ...
  output-layer name=output input=fastlstm3 include-log-softmax=false dim=$num_targets max-change=1.5 

  output-layer name=output-xent input=fastlstm3 output-delay=0 dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
