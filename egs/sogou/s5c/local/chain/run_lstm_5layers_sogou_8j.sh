...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_8job_5layers # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm4 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3
  fast-lstmp-layer name=fastlstm5 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3
