...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_cmvn_noLDA_7000h_bigchange # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=exp/chain/lstm_6j_cmvn_noLDA_7000h_ld5/egs
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm1 input=Append(-2,-1,0,1,2) cell-dim=2048 recurrent-projection-dim=512 non-recurrent-projection-dim=-1 delay=-3 
  ...
  output-layer name=output input=fastlstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=2.0
  ...
  output-layer name=output-xent input=fastlstm3 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=2.0
...
decode_suff=bigG_5epoch
graph_dir=exp/chain/lstm_6j_cmvn_noLDA_7000h_ld5/graph
if [ $stage -le 15 ]; then
