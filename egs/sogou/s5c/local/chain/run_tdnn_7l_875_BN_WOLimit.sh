...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_7l_875nodes_BN_WOlimit  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=

# training options
num_epochs=4
initial_effective_lrate=0.013
final_effective_lrate=0.00013
leftmost_questions_truncate=-1
max_param_change=5.0
...
if [ $stage -le 12 ]; then
  ...
  output-layer name=output include-log-softmax=false dim=$num_targets max-change=0.0
  ...
  output-layer name=output-xent dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=0.0
EOF
