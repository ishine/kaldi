...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_8job_h1536 # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
remove_egs=false
common_egs_dir=/search/speech/wangzhichao/kaldi/kaldi-master/egs/sogou/s5c/exp/chain/lstm_6j_ld5/egs
...
echo "Extract fbank feature finished!"
fi
...
echo "Generate lattices finished!"
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm1 cell-dim=1536 recurrent-projection-dim=384 non-recurrent-projection-dim=384 delay=-3
  fast-lstmp-layer name=fastlstm2 cell-dim=1536 recurrent-projection-dim=384 non-recurrent-projection-dim=384 delay=-3
  fast-lstmp-layer name=fastlstm3 cell-dim=1536 recurrent-projection-dim=384 non-recurrent-projection-dim=384 delay=-3
...
echo "nnet train finished!"
<<!
if [ $stage -le 14 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_bigG $dir $dir/graph_sogou
fi
!
