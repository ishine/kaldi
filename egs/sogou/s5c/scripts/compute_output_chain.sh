...
if [ $# -ne 3 ]; then
  ...
  echo "e.g.:   compute_output_chain.sh kaldi-feats tdnn-attention-model attention-output"
...
if $apply_exp; then
  ...
  output_wspecifier="ark:| copy-matrix --apply-exp ark:- ark,t:$dir/output.txt"
else
  #output_wspecifier="ark:| copy-feats --compress=$compress ark:- ark,scp:$dir/output.JOB.ark,$dir/output.JOB.scp"
  output_wspecifier="ark,t:$dir/output.txt"
fi
...
if [ $stage -le 2 ]; then
  $cmd $gpu_queue_opt JOB=1:$nj $dir/log/compute_output.log \
