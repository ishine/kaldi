...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/lstm_6j_____________ # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=
...
mfcc_data=data/train_mfcc_2500h


if [ $stage -le 9 ]; then
  # Get the alignments as lattices (gives the chain training more freedom).
  # use the same num-jobs as the alignments
  steps/align_fmllr_lats.sh --nj 30 --cmd "$train_cmd" $mfcc_data \
    data/lang exp/tri3b exp/tri3b_lats_2500h
  rm exp/tri3b_lats_2500h/fsts.*.gz # save space
fi
exit 0;
if [ $stage -le 10 ]; then
