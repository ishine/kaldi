...
/////////////////// The following code implement self-attention of full input sequence --Zhichao Wang 2019.8.19/////////////////
std::string SelfAttentionComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim=" << InputDim()
         << ", output-dim=" << OutputDim()
         << ", num-heads=" << num_heads_
...
void*
SelfAttentionComponent::Propagate(const ComponentPrecomputedIndexes *indexes_in,
                                        const CuMatrixBase<BaseFloat> &in,
                                        CuMatrixBase<BaseFloat> *out) const {
  const PrecomputedIndexes *indexes = dynamic_cast<const PrecomputedIndexes*>(
      indexes_in);
  ...
  Memo *memo = new Memo();
  memo->c.Resize(out->NumRows(), in.NumRows() * num_heads_);

  int32 query_dim = key_dim_;
  int32 input_dim_per_head = key_dim_ + value_dim_ + query_dim,
      output_dim_per_head = value_dim_;
  for (int32 h = 0; h < num_heads_; h++) {
    CuSubMatrix<BaseFloat> in_part(in, 0, in.NumRows(),
                                   h * input_dim_per_head, input_dim_per_head),
        c_part(memo->c, 0, out->NumRows(),
		       h * in.NumRows(), in.NumRows()),
        out_part(*out, 0, out->NumRows(),
                 h * output_dim_per_head, output_dim_per_head);
    PropagateOneHead(indexes->io, in_part, &c_part, &out_part);
  }
...
void SelfAttentionComponent::PropagateOneHead(
    const time_height_convolution::ConvolutionComputationIo &io,
    const CuMatrixBase<BaseFloat> &in,
    CuMatrixBase<BaseFloat> *c,
    CuMatrixBase<BaseFloat> *out) const {
  int32 query_dim = key_dim_,
      full_value_dim = value_dim_;
  KALDI_ASSERT(in.NumRows() == io.num_images * io.num_t_in &&
               out->NumRows() == io.num_images * io.num_t_out &&
               out->NumCols() == full_value_dim &&
               in.NumCols() == (key_dim_ + value_dim_ + query_dim) &&
  ...
  attention::FullAttentionForward(io, key_scale_, keys, queries, values, c, out);
}


void SelfAttentionComponent::StoreStats(
    const CuMatrixBase<BaseFloat> &, // in_value
    const CuMatrixBase<BaseFloat> &, // out_value
    void *memo_in) {
  const Memo *memo = static_cast<const Memo*>(memo_in);
  ...
  const CuMatrix<BaseFloat> &c = memo->c;
  int32 num_cols = c.NumCols() / num_heads_;

  if (entropy_stats_.Dim() != num_heads_) {
    entropy_stats_.Resize(num_heads_);
    posterior_stats_.Resize(num_heads_, num_cols);
    stats_count_ = 0.0;
  }

  if (RandInt(0, 2) == 0 || posterior_stats_.NumCols() != num_cols)
    return;  // only actually store the stats for one in three minibatches, to
             // save time.

  { // first get the posterior stats.
	CuVector<BaseFloat> c_sum(c.NumCols());
    c_sum.AddRowSumMat(1.0, c, 0.0);
    // view the vector as a matrix.
    CuSubMatrix<BaseFloat> c_sum_as_mat(c_sum.Data(), num_heads_,
                                        num_cols, num_cols);
    CuMatrix<double> c_sum_as_mat_dbl(c_sum_as_mat);
//	printf("posterior_stats matrix: %d * %d, %d, c_sum_as_mat matrix : %d * %d, %d\n", posterior_stats_.NumRows(), posterior_stats_.NumCols(), posterior_stats_.Stride(),
//	                                                                                   c_sum_as_mat_dbl.NumRows(), c_sum_as_mat_dbl.NumCols(), c_sum_as_mat_dbl.Stride() );
    posterior_stats_.AddMat(1.0, c_sum_as_mat_dbl);
  ...
  { // now get the entropy stats.
    ...
    log_c.ApplyFloor(1.0e-20);
    log_c.ApplyLog();
    CuVector<BaseFloat> dot_prod(num_heads_ * num_cols);
    dot_prod.AddDiagMatMat(-1.0, c, kTrans, log_c, kNoTrans, 0.0);
    // dot_prod is the sum over the matrix's rows (which correspond
    // to heads, and context positions), of - c * log(c), which is
    // part of the entropy.  To get the actual contribution to the
...
void SelfAttentionComponent::ZeroStats() {
  entropy_stats_.SetZero();
  posterior_stats_.SetZero();
  stats_count_ = 0.0;
}

void SelfAttentionComponent::Scale(BaseFloat scale) {
  entropy_stats_.Scale(scale);
  posterior_stats_.Scale(scale);
  stats_count_ *= scale;
}

void SelfAttentionComponent::Add(BaseFloat alpha, const Component &other_in) {
  const SelfAttentionComponent *other =
      dynamic_cast<const SelfAttentionComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  if (entropy_stats_.Dim() == 0 && other->entropy_stats_.Dim() != 0)
    entropy_stats_.Resize(other->entropy_stats_.Dim());
  if (posterior_stats_.NumRows() == 0 && other->posterior_stats_.NumRows() != 0)
  ...
  for (int32 h = 0; h < num_heads_; h++) {
    CuSubMatrix<BaseFloat>
        in_value_part(in_value, 0, in_value.NumRows(),
                      h * input_dim_per_head, input_dim_per_head),
        c_part(c, 0, out_deriv.NumRows(),
		       h * in_value.NumRows(), in_value.NumRows()),
        out_deriv_part(out_deriv, 0, out_deriv.NumRows(),
                       h * output_dim_per_head, output_dim_per_head),
        in_deriv_part(*in_deriv, 0, in_value.NumRows(),
                      h * input_dim_per_head, input_dim_per_head);
...
void SelfAttentionComponent::BackpropOneHead(
    const time_height_convolution::ConvolutionComputationIo &io,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &c,
    const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  attention::FullAttentionBackward(key_scale_, keys, queries, values, c, out_deriv,
                               &keys_deriv, &queries_deriv, &values_deriv);
}
...
void SelfAttentionComponent::GetComputationStructure(
      const std::vector<Index> &input_indexes,
      const std::vector<Index> &output_indexes,
      time_height_convolution::ConvolutionComputationIo *io) const {
  GetComputationIo(input_indexes, output_indexes, io);
...
void SelfAttentionComponent::GetInputIndexes(
  ...
  int32 first_time = output_index.t - (time_stride_ * num_left_inputs_),
      last_time = output_index.t + (time_stride_ * num_right_inputs_);
  desired_indexes->clear();
  desired_indexes->resize(num_left_inputs_ + num_right_inputs_ + 1);
  int32 n = output_index.n, x = output_index.x,
      i = 0;
  for (int32 t = first_time; t <= last_time; t += time_stride_, i++) {
    (*desired_indexes)[i].n = n;
  ...
  KALDI_ASSERT(i == num_left_inputs_ + num_right_inputs_ + 1);
}
...
bool SelfAttentionComponent::IsComputable(
  ...
  if (used_inputs != NULL) {
    int32 first_time = output_index.t - (time_stride_ * num_left_inputs_),
        last_time = output_index.t + (time_stride_ * num_right_inputs_);
    used_inputs->clear();
    used_inputs->reserve(num_left_inputs_ + num_right_inputs_ + 1);
...
// static
void SelfAttentionComponent::CreateIndexesVector(
    const std::vector<std::pair<int32, int32> > &n_x_pairs,
    int32 t_start, int32 t_step, int32 num_t_values,
    const std::unordered_set<Index, IndexHasher> &index_set,
    std::vector<Index> *output_indexes) {
...
void SelfAttentionComponent::GetIndexes(
      const std::vector<Index> &input_indexes,
      const std::vector<Index> &output_indexes,
      time_height_convolution::ConvolutionComputationIo &io,
      std::vector<Index> *new_input_indexes,
...
ComponentPrecomputedIndexes* SelfAttentionComponent::PrecomputeIndexes(
    const MiscComputationInfo &,  // misc_info
    const std::vector<Index> &input_indexes,
    const std::vector<Index> &output_indexes,
    bool) // need_backprop
...
SelfAttentionComponent::PrecomputedIndexes*
SelfAttentionComponent::PrecomputedIndexes::Copy() const {
  return new PrecomputedIndexes(*this);
}

void SelfAttentionComponent::PrecomputedIndexes::Write(
    std::ostream &os, bool binary) const {
...
void SelfAttentionComponent::PrecomputedIndexes::Read(
    std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary,
...
std::string Contextless3RestrictedAttentionComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim=" << InputDim()
         << ", output-dim=" << OutputDim()
         << ", num-heads=" << num_heads_
         ...
         << ", num-right-inputs-required=" << num_right_inputs_required_
         << ", context-dim=" << context_dim_
         << ", add-context=" << (add_context_ ? "true" : "false")
         << ", output-context=" << (output_context_ ? "true" : "false")
         << ", key-scale=" << key_scale_;
/*
  if (stats_count_ != 0.0) {
...
Contextless3RestrictedAttentionComponent::Contextless3RestrictedAttentionComponent(
    const Contextless3RestrictedAttentionComponent &other):
    num_heads_(other.num_heads_),
    key_dim_(other.key_dim_),
    value_dim_(other.value_dim_),
    context_dim_(other.context_dim_),
    ...
    num_left_inputs_required_(other.num_left_inputs_required_),
    num_right_inputs_required_(other.num_right_inputs_required_),
    add_context_(other.add_context_),
    output_context_(other.output_context_),
    key_scale_(other.key_scale_)
/*
    stats_count_(other.stats_count_),
...
void Contextless3RestrictedAttentionComponent::InitFromConfig(ConfigLine *cfl) {
  num_heads_ = 1;
  key_dim_ = -1;
  value_dim_ = -1;
  context_dim_ = -1;
  ...
  if (!ok)
    KALDI_ERR << "All of the values key-dim, value-dim must be defined" <<
        "num-left-inputs and num-right-inputs must be defined.";
  // optional arguments.
  cfl->GetValue("num-heads", &num_heads_);
  cfl->GetValue("time-stride", &time_stride_);
  cfl->GetValue("num-left-inputs-required", &num_left_inputs_required_);
  cfl->GetValue("num-right-inputs-required", &num_right_inputs_required_);
  cfl->GetValue("context-dim", &context_dim_);
  cfl->GetValue("add-context", &add_context_);
  cfl->GetValue("output-context", &output_context_);
  cfl->GetValue("key-scale", &key_scale_);
  ...
  if (num_heads_ <= 0 || key_dim_ <= 0 || value_dim_ <= 0 ||
    ...
    KALDI_ERR << "Config line contains invalid values: "
              << cfl->WholeLine();
  if (context_dim_ <= 0 && add_context_)
    KALDI_ERR << "Config line contains invalid values: "
              << cfl->WholeLine();
//  stats_count_ = 0.0;
//  context_dim_ = num_left_inputs_ + 1 + num_right_inputs_;
...
void*
Contextless3RestrictedAttentionComponent::Propagate(const ComponentPrecomputedIndexes *indexes_in,
                                        const CuMatrixBase<BaseFloat> &in,
                                        CuMatrixBase<BaseFloat> *out) const {
  const PrecomputedIndexes *indexes = dynamic_cast<const PrecomputedIndexes*>(
      indexes_in);
  KALDI_ASSERT(indexes != NULL &&
               in.NumRows() == indexes->io.num_t_in * indexes->io.num_images &&
               out->NumRows() == indexes->io.num_t_out * indexes->io.num_images &&
               indexes->io.num_t_in >= indexes->io.num_t_out);
//               ( ! add_context_ || context_dim_ == indexes->io.num_t_in ) );
  if(!add_context_){
    //SetContextDim(indexes->io.num_t_in);
    Contextless3RestrictedAttentionComponent* ptr =  const_cast<Contextless3RestrictedAttentionComponent*> (this);
    //ptr->context_dim_= indexes->io.num_t_in + num_left_inputs_ + num_right_inputs_;
    ptr->context_dim_= indexes->io.num_t_in;
  }

  Memo *memo = new Memo();
  int32 query_dim = key_dim_ + (add_context_ ? context_dim_ : 0);
  int32 input_dim_per_head = key_dim_ + value_dim_ + query_dim,
      output_dim_per_head = value_dim_ + (output_context_ ? context_dim_ : 0);
  //When reach end of sentence, chunk_size may be smaller.
  if(add_context_ && context_dim_ != indexes->io.num_t_in){
    CuMatrix<BaseFloat> in_( context_dim_ * indexes->io.num_images, in.NumCols() );
    CuSubMatrix<BaseFloat> in_sub_(in_, 0, in.NumRows(),0, in.NumCols());
    in_sub_.CopyFromMat(in);
    CuMatrix<BaseFloat> out_( context_dim_ * indexes->io.num_images, out->NumCols() );
    CuSubMatrix<BaseFloat> out_sub_(out_, 0, out->NumRows(), 0, out->NumCols());
    memo->c.Resize(out_.NumRows(), context_dim_ * num_heads_);
    for (int32 h = 0; h < num_heads_; h++) {
      CuSubMatrix<BaseFloat> in_part(in_, 0, in_.NumRows(),
                                     h * input_dim_per_head, input_dim_per_head),
          c_part(memo->c, 0, out_.NumRows(),
                 h * context_dim_, context_dim_),
          out_part(out_, 0, out_.NumRows(),
                   h * output_dim_per_head, output_dim_per_head);
      PropagateOneHead(indexes->io, in_part, &c_part, &out_part, indexes->io.num_t_in);
    }
    out->CopyFromMat(out_sub_);
  }
  else{
    memo->c.Resize(out->NumRows(), context_dim_ * num_heads_);
    for (int32 h = 0; h < num_heads_; h++) {
...
void Contextless3RestrictedAttentionComponent::PropagateOneHead(
    const time_height_convolution::ConvolutionComputationIo &io,
    const CuMatrixBase<BaseFloat> &in,
    CuMatrixBase<BaseFloat> *c,
    CuMatrixBase<BaseFloat> *out,
    int32 effective_context_dim) const {
  int32 query_dim = key_dim_ + (add_context_ ? context_dim_ : 0),
      full_value_dim = value_dim_ + (output_context_ ? context_dim_ : 0);
  KALDI_ASSERT(in.NumRows() == io.num_images * io.num_t_in &&
               out->NumRows() == io.num_images * io.num_t_out &&
  ...
  // that don't appear in the output.
  int32 steps_left_context = (io.start_t_out - io.start_t_in) / io.t_step_in,
  //int32 steps_left_context = num_left_inputs_,
      rows_left_context = steps_left_context * io.num_images;
  KALDI_ASSERT(rows_left_context >= 0);
  ...
  CuSubMatrix<BaseFloat> queries(in, rows_left_context, out->NumRows(),
                                 key_dim_ + value_dim_, query_dim);
  //CuSubMatrix<BaseFloat> queries(in, 0, in.NumRows(), key_dim_ + value_dim_, query_dim);
  // 'keys' contains the keys; note, these are not extended with
  // context information; that happens further in.
  CuSubMatrix<BaseFloat> keys(in, 0, in.NumRows(), 0, key_dim_);
  ...
  contextless3attention::AttentionForward(key_scale_, keys, queries, values, c, out, io.num_images, effective_context_dim);
}

/*
void Contextless3RestrictedAttentionComponent::StoreStats(
    const CuMatrixBase<BaseFloat> &, // in_value
    const CuMatrixBase<BaseFloat> &, // out_value
    void *memo_in) {
  const Memo *memo = static_cast<const Memo*>(memo_in);
...
void Contextless3RestrictedAttentionComponent::ZeroStats() {
  entropy_stats_.SetZero();
  posterior_stats_.SetZero();
  stats_count_ = 0.0;
}

void Contextless3RestrictedAttentionComponent::Scale(BaseFloat scale) {
  entropy_stats_.Scale(scale);
  posterior_stats_.Scale(scale);
  stats_count_ *= scale;
}

void Contextless3RestrictedAttentionComponent::Add(BaseFloat alpha, const Component &other_in) {
  const Contextless3RestrictedAttentionComponent *other =
      dynamic_cast<const Contextless3RestrictedAttentionComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  if (entropy_stats_.Dim() == 0 && other->entropy_stats_.Dim() != 0)
    entropy_stats_.Resize(other->entropy_stats_.Dim());
  if (posterior_stats_.NumRows() == 0 && other->posterior_stats_.NumRows() != 0)
...
void Contextless3RestrictedAttentionComponent::Check() const {
  KALDI_ASSERT(num_heads_ > 0 && key_dim_ > 0 && value_dim_ > 0 &&
               num_left_inputs_ >= 0 && num_right_inputs_ >= 0 &&
               (num_left_inputs_ + num_right_inputs_) >= 0 &&
               time_stride_ > 0 &&
...
void Contextless3RestrictedAttentionComponent::Backprop(
    const std::string &debug_info,
    const ComponentPrecomputedIndexes *indexes_in,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &, // out_value
  ...
  const time_height_convolution::ConvolutionComputationIo &io = indexes->io;
  //if(!add_context_)
  //  SetContextDim(indexes->io.num_t_out);
  if(!add_context_){
    //SetContextDim(indexes->io.num_t_in);
    Contextless3RestrictedAttentionComponent* ptr =  const_cast<Contextless3RestrictedAttentionComponent*> (this);
    //ptr->context_dim_= indexes->io.num_t_in + num_left_inputs_ + num_right_inputs_;
  ...
  KALDI_ASSERT(indexes != NULL &&
               ...
               out_deriv.NumRows() == io.num_t_out * io.num_images &&
               in_deriv != NULL && SameDim(in_value, *in_deriv));
//               ( ! add_context_ || context_dim_ == indexes->io.num_t_out ) );
  ...
  if(add_context_ && context_dim_ != indexes->io.num_t_in){
    CuMatrix<BaseFloat> in_value_( context_dim_ * indexes->io.num_images, in_value.NumCols() );
    CuSubMatrix<BaseFloat> in_value_sub_(0, in_value.NumRows(),0, in_value.NumCols());
    in_value_sub_.CopyFromMat(in_value);
    CuMatrix<BaseFloat> out_deriv_( context_dim_ * indexes->io.num_images, out_deriv.NumCols() );
    CuSubMatrix<BaseFloat> out_deriv_sub_(out_deriv_, 0, out_deriv.NumRows(), 0, out_deriv.NumCols());
    out_deriv_sub_.CopyFromMat(out_deriv);
    CuMatrix<BaseFloat> in_deriv_( context_dim_ * indexes->io.num_images, in_deriv->NumCols() );
    CuSubMatrix<BaseFloat> in_deriv_sub_(in_deriv_, 0, in_deriv->NumRows(), 0, in_deriv->NumCols());
    for (int32 h = 0; h < num_heads_; h++) {
      CuSubMatrix<BaseFloat>
          in_value_part(in_value_, 0, in_value_.NumRows(),
                        h * input_dim_per_head, input_dim_per_head),
          c_part(c, 0, out_deriv_.NumRows(),
                 h * context_dim_, context_dim_),
          out_deriv_part(out_deriv_, 0, out_deriv_.NumRows(),
                         h * output_dim_per_head, output_dim_per_head),
          in_deriv_part(in_deriv_, 0, in_value_.NumRows(),
                        h * input_dim_per_head, input_dim_per_head);
      BackpropOneHead(io, in_value_part, c_part, out_deriv_part,
                      &in_deriv_part, indexes->io.num_t_in);
    }
    in_deriv->CopyFromMat(in_deriv_sub_);
  }
  else {
    for (int32 h = 0; h < num_heads_; h++) {
      CuSubMatrix<BaseFloat>
...
void Contextless3RestrictedAttentionComponent::BackpropOneHead(
    const time_height_convolution::ConvolutionComputationIo &io,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &c,
    const CuMatrixBase<BaseFloat> &out_deriv,
...
/*
  CuSubMatrix<BaseFloat> queries(in_value, rows_left_context, out_deriv.NumRows(),
                                 key_dim_ + value_dim_, query_dim),
      queries_deriv(*in_deriv, 0, in_value.NumRows(),
                    key_dim_ + value_dim_, query_dim),
      keys(in_value, 0, in_value.NumRows(), 0, key_dim_),
      keys_deriv(*in_deriv,  0, in_value.NumRows(), 0, key_dim_),
      values(in_value, 0, in_value.NumRows(), key_dim_, value_dim_),
...
*/
  ...
  contextless3attention::AttentionBackward(key_scale_, keys, queries, values, c, out_deriv,
                               &keys_deriv, &queries_deriv, &values_deriv, io.num_images, effective_context_dim);
}



void Contextless3RestrictedAttentionComponent::ReorderIndexes(
    std::vector<Index> *input_indexes,
    std::vector<Index> *output_indexes) const {
  using namespace time_height_convolution;
  ConvolutionComputationIo io;
...
void Contextless3RestrictedAttentionComponent::GetComputationStructure(
      const std::vector<Index> &input_indexes,
      const std::vector<Index> &output_indexes,
      time_height_convolution::ConvolutionComputationIo *io) const {
  GetComputationIo(input_indexes, output_indexes, io);
...
void Contextless3RestrictedAttentionComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<Contextless3RestrictedAttentionComponent>");
  WriteToken(os, binary, "<NumHeads>");
  WriteBasicType(os, binary, num_heads_);
  WriteToken(os, binary, "<KeyDim>");
  WriteBasicType(os, binary, key_dim_);
  ...
  WriteToken(os, binary, "<NumRightInputsRequired>");
  WriteBasicType(os, binary, num_right_inputs_required_);
  WriteToken(os, binary, "<ContextDim>");
  WriteBasicType(os, binary, context_dim_);
  WriteToken(os, binary, "<AddContext>");
  WriteBasicType(os, binary, add_context_);
  WriteToken(os, binary, "<OutputContext>");
  WriteBasicType(os, binary, output_context_);
  WriteToken(os, binary, "<KeyScale>");
  WriteBasicType(os, binary, key_scale_);
...
void Contextless3RestrictedAttentionComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<Contextless3RestrictedAttentionComponent>",
                       "<NumHeads>");
  ReadBasicType(is, binary, &num_heads_);
  ExpectToken(is, binary, "<KeyDim>");
  ReadBasicType(is, binary, &key_dim_);
  ...
  ExpectToken(is, binary, "<NumRightInputsRequired>");
  ReadBasicType(is, binary, &num_right_inputs_required_);
  ExpectToken(is, binary, "<ContextDim>");
  ReadBasicType(is, binary, &context_dim_);
  ExpectToken(is, binary, "<AddContext>");
  ReadBasicType(is, binary, &add_context_);
  ExpectToken(is, binary, "<OutputContext>");
  ReadBasicType(is, binary, &output_context_);
  ExpectToken(is, binary, "<KeyScale>");
  ReadBasicType(is, binary, &key_scale_);
/*
  ...
  posterior_stats_.Read(is, binary);
*/
  ExpectToken(is, binary, "</Contextless3RestrictedAttentionComponent>");

  //context_dim_ = num_left_inputs_ + 1 + num_right_inputs_;
}

void Contextless3RestrictedAttentionComponent::GetInputIndexes(
    const MiscComputationInfo &misc_info,
    const Index &output_index,
    std::vector<Index> *desired_indexes) const {
  KALDI_ASSERT(output_index.t != kNoTime);
  ...
  desired_indexes->clear();
  //desired_indexes->resize(context_dim_);
  desired_indexes->resize((last_time - first_time + time_stride_) / time_stride_);
  int32 n = output_index.n, x = output_index.x,
      i = 0;
  for (int32 t = first_time; t <= last_time; t += time_stride_, i++) {
    (*desired_indexes)[i].n = n;
...
bool Contextless3RestrictedAttentionComponent::IsComputable(
    const MiscComputationInfo &misc_info,
    const Index &output_index,
    const IndexSet &input_index_set,
    std::vector<Index> *used_inputs) const {
  ...
  if (used_inputs != NULL) {
    int32 first_time = output_index.t - (time_stride_ * num_left_inputs_),
        last_time = output_index.t + (time_stride_ * num_right_inputs_);
    used_inputs->clear();
    used_inputs->reserve((last_time - first_time + time_stride_)/time_stride_);
...
// static
void Contextless3RestrictedAttentionComponent::CreateIndexesVector(
    const std::vector<std::pair<int32, int32> > &n_x_pairs,
    int32 t_start, int32 t_step, int32 num_t_values,
    const std::unordered_set<Index, IndexHasher> &index_set,
    std::vector<Index> *output_indexes) {
...
void Contextless3RestrictedAttentionComponent::GetIndexes(
      const std::vector<Index> &input_indexes,
      const std::vector<Index> &output_indexes,
      time_height_convolution::ConvolutionComputationIo &io,
      std::vector<Index> *new_input_indexes,
...
ComponentPrecomputedIndexes* Contextless3RestrictedAttentionComponent::PrecomputeIndexes(
    const MiscComputationInfo &,  // misc_info
    const std::vector<Index> &input_indexes,
    const std::vector<Index> &output_indexes,
    bool) // need_backprop
...
Contextless3RestrictedAttentionComponent::PrecomputedIndexes*
Contextless3RestrictedAttentionComponent::PrecomputedIndexes::Copy() const {
  return new PrecomputedIndexes(*this);
}

void Contextless3RestrictedAttentionComponent::PrecomputedIndexes::Write(
    std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<Contextless3RestrictedAttentionComponentPrecomputedIndexes>");
  WriteToken(os, binary, "<Io>");
  io.Write(os, binary);
  WriteToken(os, binary, "</Contextless3RestrictedAttentionComponentPrecomputedIndexes>");
}

void Contextless3RestrictedAttentionComponent::PrecomputedIndexes::Read(
    std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary,
                       "<Contextless3RestrictedAttentionComponentPrecomputedIndexes>",
                       "<Io>");
  io.Read(is, binary);
  ExpectToken(is, binary, "</Contextless3RestrictedAttentionComponentPrecomputedIndexes>");
}
