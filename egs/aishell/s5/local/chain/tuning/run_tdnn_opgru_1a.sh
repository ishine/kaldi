...
#           2017 Tsinghua University (THU) Lu Huang
# Apache 2.0

# This is the first attempt to try TDNN-OPGRU.
...
train_stage=-10
get_egs_stage=-10
dir=exp/chain/tdnn_opgru_1a # Note: _sp will get added to this if 
...
if [ $stage -le 7 ]; then
  ...
  rm exp/exp/tri5a_sp_lats/fsts.*.gz # save space
fi
...
if [ $stage -le 10 ]; then
  ...
  opgru-layer name=opgru1 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 
  relu-batchnorm-layer name=tdnn4 input=Append(-3,0,3) dim=625
  relu-batchnorm-layer name=tdnn5 input=Append(-3,0,3) dim=625
  opgru-layer name=opgru2 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 
  relu-batchnorm-layer name=tdnn6 input=Append(-3,0,3) dim=625
  relu-batchnorm-layer name=tdnn7 input=Append(-3,0,3) dim=625
  opgru-layer name=opgru3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 
