...
cat > $nnet_proto <<EOF
<AffineTransform> <InputDim> 40 <OutputDim> 512 <BiasMean> -2.000000 <BiasRange> 4.000000 <ParamStddev> 0.210675 <MaxNorm> 0.000000
<BiCompactVfsmn> <InputDim> 512 <OutputDim> 512 <BackOrder> 10 <AheadOrder> 10
...
for iter in $(seq -w $max_iters); do
  ...
  nnet-train-fsmn --cross-validate=false --randomize=false --verbose=0 --minibatch-size=64 --learn-rate=0.00004 --momentum=0.9 --l1-penalty=0 --l2-penalty=0 "scp:$DEV_DATA/dev.scp" "ark:ali-to-post ark,t:$DEV_DATA/dev.ali ark:- |" $nnet_last $nnet_next 2>> $log
  nnet_last=$nnet_next
done
