...
for dir in $*; do
  for f in phones.txt; do
    diff $first_src/lang/$f $dir/lang/$f 1>/dev/null 2>&1
    if [ $? -ne 0 ]; then
      echo "$0: Cannot combine denlats directories with different $f files."
    fi
  done
done

for f in frame_subsampling_factor cmvn_opts num_jobs $extra_files; do
