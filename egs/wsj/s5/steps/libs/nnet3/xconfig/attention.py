...
# This class is for parsing lines like
#  'selfattention-renorm-layer num-heads=10 value-dim=50 key-dim=50 time-stride=3 num-left-inputs=5 num-right-inputs=2.'
#
# Parameters of the class, and their defaults:
#   input='[-1]'               [Descriptor giving the input of the layer.]
#   self-repair-scale=1.0e-05  [Affects relu, sigmoid and tanh layers.]
...
class XconfigSelfAttentionLayer(XconfigLayerBase):
    def __init__(self, first_token, key_to_value, prev_names = None):
        # Here we just list some likely combinations.. you can just add any
        # combinations you want to use, to this list.
        assert first_token in ['selfattention-layer']
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _add_components(self, input_desc, input_dim, nonlinearities):
        ...
        for nonlinearity in nonlinearities:
            ...
            elif nonlinearity == 'selfattention':
                line = ('component name={0}.{1}'
                        ' type=SelfAttentionComponent'
                        ' value-dim={2}'
                        ' key-dim={3}'
                        ' num-left-inputs={4}'
                        ' num-right-inputs={5}'
...
class XconfigPositionEmbeddingLayer(XconfigLayerBase):
    def __init__(self, first_token, key_to_value, prev_names = None):
        # Here we just list some likely combinations.. you can just add any
        # combinations you want to use, to this list.
        assert first_token in ['positionembedding-layer']
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def set_default_configs(self):
        ...
        self.config = { 'input':'[-1]',
                        'dim': -1 }
    def check_configs(self):
        if self.config['dim'] <= 0.0:
            raise RuntimeError("dim has invalid value {0}"
                               .format(self.config['dim']))
    def output_name(self, auxiliary_output=None):
        # at a later stage we might want to expose even the pre-nonlinearity
        # vectors
        assert auxiliary_output == None
        return '{0}.positionembedding'.format(self.name)
    def output_dim(self, auxiliary_output = None):
    ...
    def _add_components(self, input_desc, input_dim, nonlinearities):
        configs = []
        line = ('component name={0}.positionembedding'
                ' type=PositionEmbeddingComponent'
                ' input-dim={1}'
                ' output-dim={2}'
                ''.format(self.name, input_dim, input_dim))
        configs.append(line)
        line = ('component-node name={0}.positionembedding'
                ' component={0}.positionembedding input={1}'
                ''.format(self.name, input_desc))
        configs.append(line)
        return configs

class XconfigContextlessAttentionLayer(XconfigLayerBase):
    def __init__(self, first_token, key_to_value, prev_names = None):
        # Here we just list some likely combinations.. you can just add any
        # combinations you want to use, to this list.
        assert first_token in ['contextlessattention-layer']
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def attention_input_dim(self):
        context_dim = (self.config['num-left-inputs'] +
                       self.config['num-right-inputs'] + 0)
        num_heads = self.config['num-heads']
        key_dim = self.config['key-dim']
        value_dim = self.config['value-dim']
        query_dim = key_dim;
    ...
    def _add_components(self, input_desc, input_dim, nonlinearities):
        ...
        for nonlinearity in nonlinearities:
            ...
            elif nonlinearity == 'contextlessattention':
                line = ('component name={0}.{1}'
                        ' type=Contextless3RestrictedAttentionComponent'
                        ' value-dim={2}'
                        ' key-dim={3}'
                        ' num-left-inputs={4}'
                        ' num-right-inputs={5}'
                        ...
                        ''.format(self.name, nonlinearity,
                                  ...
                                  self.config['num-right-inputs-required'],
                                  self.config['output-context'],
                                  self.config['add-context'],
                                  self.config['time-stride'],
                                  self.config['num-heads'],
                                  self.config['key-scale'],
                                  self.config['context-dim']))
                dim = self.attention_output_dim()
