...
labels_tr_file=`echo $labels_tr | awk -F "scp:| " '{print $3;}'`
...
if [ 0 == $warmup ];then
  ...
  $warmup_tool \
   ...
   --num-threads=1 --use-psgd=$use_psgd --asgd-lock=$asgd_lock \
   --binary=true \
...
for iter in $(seq -w $max_iters); do
  ...
  # sds
  {
     ...
     awk 'NR==FNR{if($1 in ss){} else ss[$1]=$0;}NR>FNR{if($1 in ss)print ss[$1];}' $labels_tr_file $tr_sds > $ali_tr_sds
  ...
  if [ 1 == $do_svd -a $iter -eq 1 ];then
	...
	nnet-svd --binary=false --min-rank=128 --svd-layers=$svd_rate:$svd_rate:$svd_rate:$svd_rate:$svd_rate $mlp_best $mlp_svd 2>$log || { cat $log; exit 1; }
	mlp_best=$mlp_svd
