...
parallel_opts="-pe smp 6" # use 2 CPUs (1 DNN-forward, 1 decoder)
...
feats_scp="ark:apply-cmvn --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- | add-deltas ark:- ark:- |"
...
if ! $skip_scoring ; then
  [ ! -x local/score_swbd.sh ] && \
    echo "Not scoring because local/score.sh does not exist or not executable." && exit 1;
  local/score_swbd.sh $scoring_opts --cmd "$decode_cmd" $data $graphdir $dir || exit 1;
fi
