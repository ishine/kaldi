...
for f in $data/feats.scp $alidir/{tree,final.mdl,ali.1.ark} $denlatdir/lat.scp $srcdir/{final.nnet,final.feature_transform}; do
...
cp $dir/train.scp $dir/scplist/train.scp
[ ! -e $data/featlen.scp ] && feat-to-len scp:$dir/train.scp ark,t:$data/featlen.scp
steps/nnet/seq_split.sh $jbs $merge_size $dir/scplist/train.scp $data/featlen.scp $denlatdir/lat.scp
feats="ark,o:copy-feats scp:$dir/scplist/train.scp ark:- |"
...
while [ $x -le $num_iters ]; do
  ...
  if [ -f $dir/$x.nnet ]; then
	...
	logfile=$dir/log/mmi_mpi.$x.log
    $cmd $dir/log/mmi.$x.log \
     mpiexec -n $jbs -f hostfile -wdir . nnet-train-sequential-parallel-mpi \
