...
#nnet_forward_opts=" --apply-log=true --prior-scale=1.0 --num-stream=40 --batch-size=20 --num-threads=1"
...
[ ! -z "$cmvn_opts" ] && feats="$feats apply-cmvn $cmvn_opts scp:$sdata/JOB/cmvn.scp ark:- ark:- |"
# add-deltas (optional),
...
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    /sgfs//users/wd007/src/eesen/src/decoderbin/latgen-faster  --min-active=$min_active --max-active=$max_active --max-mem=$max_mem --beam=$beam \
