...
verbose=1
# tool
train_tool="lm-train-lstm-lm-parallel-mpi --global-momentum=0.90 --num-stream=64 --batch-size=12 --targets-delay=0 --objective-function=cbxent"
...
merge_size=120000
merge_function=globalgradient #average #average #globalada #average globalsum
use_psgd=false
num_process=2
num_threads=4
asgd_lock=true
...
train_tool="mpiexec -n $num_process -f hostfile -wdir . "$train_tool
mlp_init=$1
feats_tr=$2
feats_cv=$3
dir=$4
...
#[ ! -e $cv_len ] && awk '{print $1,NF-2;}' $cv_file > $cv_len 
#[ ! -e $tr_len ] && awk '{print $1,NF-2;}' $tr_file > $tr_len
...
steps/nnet/lm_split_mpi.sh $num_process $merge_size $cv_file_mpi $cv_len "cv" || exit 1;
...
steps/nnet/lm_split_mpi.sh $num_process $merge_size $tr_file_mpi $tr_len "tr" || exit 1;
...
$train_tool --cross-validate=true \
 ...
 --num-threads=$num_threads --merge-size=$merge_size --merge-function=$merge_function --log-file=$log \
 ${feature_transform:+ "--feature-transform=$feature_transform"} \
 ${frame_weights:+ "--frame-weights=$frame_weights"} \
 ${class_boundary:+ "--class-boundary=$class_boundary"} \
 ${var_penalty:+ "--var-penalty=$var_penalty"} \
...
for iter in $(seq -w $max_iters); do
  ...
  # sds
  {
     ...
     steps/nnet/lm_split_mpi.sh $num_process $merge_size $tr_sds $tr_len "tr"
  ...
  $train_tool --cross-validate=true \
   --minibatch-size=$minibatch_size --randomizer-size=$randomizer_size --randomize=false --verbose=$verbose \
   --num-threads=$num_threads --merge-size=$merge_size --use-psgd=$use_psgd --merge-function=$merge_function --log-file=$log \
   ${feature_transform:+ "--feature-transform=$feature_transform"} \
   ${frame_weights:+ "--frame-weights=$frame_weights"} \
   ${class_boundary:+ "--class-boundary=$class_boundary"} \
   "$feats_cv" $mlp_next \
  ...
  if [ 1 == $do_svd -a $iter -eq 1 ];then
	...
	nnet-svd --binary=false --min-rank=256 --svd-layers=1:0.50:0.50:0.50:0.50:0.50:0.50 $mlp_best $mlp_svd 2>$log || { cat $log; exit 1; }
	mlp_best=$mlp_svd
	n=1; minibatch_size=256; sds_rate=1
