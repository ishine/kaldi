...
compress=false
#cmd=run.pl
#cmd="queue.pl -l hostname='(beijing|changsha|shenzhen|shanghai)' "
cmd=queue.pl
nj=50
mj=50
...
utils/split_scp.pl $dir/$wavscp $split_scps || exit 1;
 ...
 $cmd --max-jobs-run $mj JOB=1:$nj $dir/wav2fbank.JOB.log \
	compute-fbank-feats --verbose=1 --config=$fbank_config scp,p:$dir/wav.JOB.scp ark:- \| \
    		copy-feats --compress=$compress ark:- ark,scp:`pwd`/$dir/raw_feat.JOB.ark,`pwd`/$dir/raw_feat.JOB.scp  || exit 1;
