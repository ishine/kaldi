...
if [ $# != 3 ]; then
   echo "usage: make_mfcc.sh <mfcc_config> <data-dir> <path-to-mfccdir>";
   exit 1;
fi
...
compute-mfcc-feats --verbose=1 --config=$mfcc_config scp,p:$dir/$wavscp ark:- | \
    copy-feats --compress=$compress ark:- \
     ark,scp:`pwd`/$dir/feats.ark,`pwd`/$dir/feats.scp  > $dir/mfcc".log" 2>&1 2>&1 || exit 1;
