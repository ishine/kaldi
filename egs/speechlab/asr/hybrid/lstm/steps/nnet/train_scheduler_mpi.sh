...
# tool
# tool
train_tool="mpiexec -n 8 -f hostfile -wdir . nnet-train-lstm-streams-parallel-mpi --global-momentum=0.90 --num-stream=40 --batch-size=20 --targets-delay=5"
...
if [ 0 == $warmup ];then
   ...
   wn=`wc -l $tr_file_mpi | awk '{num=int($1/10);if(num>900000)num=900000;else if(num<300000)num=300000; print num;}' `
  ...
  $warmup_tool \
   ...
   --num-threads=4 --use-psgd=$use_psgd --asgd-lock=$asgd_lock --binary=true \
...
for iter in $(seq -w $max_iters); do
  ...
  $train_tool \
   ...
   --skip-frames=$skip_frames --sweep-frames=$sweep_frames --sweep-loop=$sweep_loop --skip-inner=$skip_inner \
   --minibatch-size=$minibatch_size --randomizer-size=$randomizer_size --randomize=true --verbose=$verbose \
   --num-threads=$num_threads --merge-size=$merge_size --use-psgd=$use_psgd --merge-function=$merge_function --log-file=$log --binary=true \
   ${kld_scale:+ --kld-scale="$kld_scale"} \
   ${si_model:+ --si-model="$si_model"} \
   ${feature_transform:+ --feature-transform=$feature_transform} \
   ${frame_weights:+ "--frame-weights=$frame_weights"} \
