...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/pretrain_dbn.log \
    steps/nnet/pretrain_dbn.sh --hid-dim 2048 --rbm-iter 20 $data_fmllr/train $dir || exit 1;
fi
}
...
cuda_cmd=run.pl
data_fmllr=data/train
ali=exp/tri3b_gmm_ali
dir=exp/cnn_relu_baseline
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train_mpi.sh --nnet-init $dir/nnet.init --learn-rate 0.00003 --feature-transform $feature_transform \
    --delta_opts "--delta-order=2" --cmvn-opts "--norm-means=true --norm-vars=false" \
    --train-tool "mpiexec -n 8 -f hostfile -wdir . nnet-train-frmshuff-parallel-mpi --global-momentum=0.90" \
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/lang $ali $ali $dir || exit 1;
fi

   # --train-tool "mpiexec -n 4 -f hostfile -wdir . nnet-train-frmshuff-parallel-mpi --global-momentum=0.5" \
   # 0.5 0.75
}

lang=data/lang_decode
data=/aifs/users/wd007/asr/test_set/aicloud
#ai_mcsnor_evl13jun_v1  ai_mcsntr_evl13mar_v1_1  ai_mcsntr_evl14jan_v1  ai_mcsntr_evl14mar_v1
#ai_mcsntr_evl13mar_v1  ai_mcsnur_evl13jan_v1  chn_sougou_150529  local_navi
decode_cmd=run.pl
