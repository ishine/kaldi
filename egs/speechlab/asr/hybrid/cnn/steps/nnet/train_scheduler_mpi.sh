...
verbose=1
# tool
train_tool="mpiexec -n 4 -f hostfile -wdir . nnet-train-frmshuff-parallel-mpi"
...
steps/nnet/dnn_split.sh $num_jobs $merge_size $cv_file_mpi $cv_len $cv_ali "cv"
...
steps/nnet/dnn_split.sh $num_jobs $merge_size $tr_file_mpi $tr_len $tr_ali "tr"
...
if [ 0 == $warmup ];then
   ...
   num_warmup=`wc -l $dir/scplist/train_sds.scp | awk '{num=int($1/30)*3;if(num>300000)num=300000;else if(num<300000)num=300000; print num;}' `
...
for iter in $(seq -w $max_iters); do
  ...
  # sds
  {
     ...
     num_warmup=0
     steps/nnet/dnn_split.sh $num_jobs $merge_size $tr_sds $tr_len $tr_ali "tr"
  ...
  if [ 1 == $do_svd -a $iter -eq 1 ];then
	...
	nnet-svd --binary=false --min-rank=256 --svd-layers=$svd_rate:$svd_rate:$svd_rate:$svd_rate:$svd_rate $mlp_best $mlp_svd 2>$log || { cat $log; exit 1; }
	mlp_best=$mlp_svd
	n=1; minibatch_size=$min_minibatch_size; sds_rate=1.0;
