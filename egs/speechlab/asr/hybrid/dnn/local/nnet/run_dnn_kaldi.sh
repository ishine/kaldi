...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/pretrain_dbn.log \
    steps/nnet/pretrain_dbn.sh --hid-dim 2048 --rbm-iter 2 \
	--delta_opts "--delta-order=2" --cmvn-opts "--norm-means=true --norm-vars=false" \
	$data_fmllr/train_tr90 $dir || exit 1;
fi
}

ali=exp/tri_gmm_ali
dbndir=exp/dnn_pretrain-dbn
dir=exp/dnn_baseline
false && \
{
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train_asgd.sh --copy-feats false --feature-transform $feature_transform --dbn $dbn --hid-layers 0 \
	--learn-rate 0.0008 --train-opts "--verbose 1" --delta_opts "--delta-order=2" --cmvn-opts "--norm-means=true --norm-vars=false" \
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/lang $ali $ali $dir || exit 1;
fi
}
