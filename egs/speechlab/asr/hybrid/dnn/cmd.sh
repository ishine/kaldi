...
#HOSTS=" -l hostname='(chengdu|taiyuan|changchun)' "
train_cmd="queue.pl -l arch=*64"$HOSTS
decode_cmd="queue.pl -l arch=*64"$HOSTS
...
cuda_cmd="queue.pl -l arch=*64 "$HOSTS
...
if [ ${host#*.} == "fit.vutbr.cz" ]; then
  ...
  export cuda_cmd="queue.pl -q $gpu_queue -l gpu=1"$HOSTS
fi
