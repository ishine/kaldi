...
acwt=0.9 # note: only really affects pruning (scoring is on lattices).
beam=15.0
lattice_beam=8.0
min_active=200
max_active=7000 # limit of active tokens
max_mem=50000000 # approx. limit to memory consumption during minimization in bytes
blank_posterior_scale=0.11
nnet_forward_opts=" --apply-log=true --prior-scale=1.0 --num-stream=40 --batch-size=20 --num-threads=1 --copy-posterior=false" #--prior-scale=1.0
#nnet_forward_opts=" --apply-log=true --prior-scale=1.0 "

skip_scoring=false
#scoring_opts="--min-lmwt 5 --max-lmwt 10 --acwt-factor 0.1"
...
[ -z "$class_frame_counts" ] && class_frame_counts=$srcdir/label.counts

# Check that files exist
for f in $graphdir/TLG.fst $sdata/1/feats.scp $nnet $feature_transform $class_frame_counts ; do
...
phone_determinize=false
#--class-frame-counts=$class_frame_counts
#/sgfs//users/wd007/src/eesen/src/decoderbin/latgen-faster 
#/aifs/users/tx078/kaldi-try/src/nnetbin/nnet-forward

#--class-frame-counts=$class_frame_counts
#--blank-posterior-scale=$blank_posterior_scale
# Run the decoding in the queue,
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    nnet-forward-parallel $nnet_forward_opts --class-frame-counts=$class_frame_counts --feature-transform=$feature_transform --use-gpu=$use_gpu $nnet "$feats" ark:- \| \
    latgen-faster-ctc$thread_string --min-active=$min_active --max-active=$max_active --max-mem=$max_mem --beam=$beam \
...
if ! $skip_scoring ; then
  if [ -f $data/stm ]; then # use sclite scoring.
    ...
    local/score_ctc.sh $scoring_opts --cmd "$cmd" $data $graphdir $dir || exit 1;
  fi    
fi
