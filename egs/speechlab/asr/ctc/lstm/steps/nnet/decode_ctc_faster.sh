...
acwt=0.6 # note: only really affects pruning (scoring is on lattices). acoustic-scale
beam=13.0
min_active=20
...
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    nnet-forward-parallel $nnet_forward_opts --feature-transform=$feature_transform --blank-posterior-scale=$blank_posterior_scale --use-gpu=$use_gpu $nnet "$feats" ark:- \| \
    decode-faster-ctc --min-active=$min_active --max-active=$max_active --beam=$beam \
    --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt \
    $graphdir/TLG.fst ark:- ark,t:$dir/trans.JOB || exit 1;
fi
