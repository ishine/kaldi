...
# tool
# tool
train_tool="mpiexec -n 4 -f hostfile -wdir . nnet-train-ctc-parallel-mpi --global-momentum=0.75 --num-stream=40 --batch-size=20 --targets-delay=5"
warmup_tool="nnet-train-ctc-parallel-mpi --num-stream=40 --batch-size=20 --targets-delay=5"
frame_weights=
...
steps/nnet/ctc_split.sh $num_jobs $merge_size $skip_frames $cv_file_mpi $cv_len $cv_ali "cv" || exit 1
cv_file_mpi=$dir/scplist/cv.JOB.scp
feats_cv=`echo $feats_cv | awk  -v f1=$cv_file -v f2=$cv_file_mpi 'BEGIN{FS=f1}{print $1f2$2;}'`
echo steps/nnet/ctc_split.sh $num_jobs $merge_size $skip_frames $cv_file_mpi $cv_len $cv_ali "cv" || exit 1

tr_file_mpi=$dir/scplist/train_sds.scp
cp $tr_file $tr_file_mpi
steps/nnet/ctc_split.sh $num_jobs $merge_size $skip_frames $tr_file_mpi $tr_len $tr_ali "tr" || exit 1
...
for iter in $(seq -w $max_iters); do
  ...
  # sds
  {
     ...
     steps/nnet/ctc_split.sh $num_jobs $merge_size $skip_frames $tr_sds $tr_len $tr_ali "tr" || exit 1
