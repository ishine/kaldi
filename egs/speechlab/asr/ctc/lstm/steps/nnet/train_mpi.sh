...
[  -e $data/featlen.scp ] && cp $data/featlen.scp  $dir/tr_len.scp
[  -e $data_cv/featlen.scp ] && cp $data_cv/featlen.scp  $dir/cv_len.scp
  #make lattice scp with same order as the shuffled feature scp
  awk '{if(r==0)aliH[$1]=$0; if(r==1){if($1 in aliH) print aliH[$1];} }' $alidir/ali.scp r=1 $dir/train.scp > $dir/ali.tr.scp
  awk '{if(r==0)aliH[$1]=$0; if(r==1){if($1 in aliH) print aliH[$1];} }' $alidir/ali.scp r=1 $dir/cv.scp > $dir/ali.cv.scp
  ...
  mv $dir/cv.scp $dir/cv.scp_unfilt
  awk '{if(r==0)aliH[$1]=1; if(r==1){if($1 in aliH) print $0;} }' $alidir/ali.tr.scp r=1 $dir/train.scp_unfilt > $dir/train.scp
  awk '{if(r==0)aliH[$1]=1; if(r==1){if($1 in aliH) print $0;} }' $alidir/ali.cv.scp r=1 $dir/cv.scp_unfilt > $dir/cv.scp
...
head -n 1000000 $dir/train.scp > $dir/train.scp.10k
...
if [ ! -z "$labels" ]; then
  ...
  #labels_tr="ark,o:ali-to-pdf $alidir/final.mdl scp:$dir/scplist/ali.tr.scp ark:- | ali-to-post ark:- ark:- |"
  #labels_cv="ark,o:ali-to-pdf $alidir/final.mdl scp:$dir/scplist/ali.cv.scp ark:- | ali-to-post ark:- ark:- |"
  labels_tr="ark,o:ali-to-post scp:$dir/scplist/ali.tr.JOB.scp ark:- |"
  labels_cv="ark,o:ali-to-post scp:$dir/scplist/ali.cv.JOB.scp ark:- |"
  # 
  ...
  #labels_tr_pdf="ark:ali-to-pdf $alidir/final.mdl scp:$dir/ali.tr.scp ark:- |" # for analyze-counts.
  #labels_tr_phn="ark:ali-to-phones --per-frame=true $alidir/final.mdl scp:$dir/ali.tr.scp ark:- |"
  labels_tr_pdf="ark:copy-align scp:$alidir/ali.tr.scp ark:- |"
  ...
  # [ -e $lang/phones.txt ] && [ -e $dir/log/analyze_counts_phones.log ] || analyze-counts --verbose=1 --symbol-table=$lang/phones.txt "$labels_tr_phn" /dev/null 2>$dir/log/analyze_counts_phones.log || exit 1
fi
...
if [ ! -z "$skip_frames" ]; then
  skip_opts="--skip-frames=$skip_frames --skip-inner=$skip_inner"
...
steps/nnet/train_scheduler_mpi.sh \
  ...
  ${skip_frames:+ --skip-frames "$skip_frames"} \
  ${skip_inner:+ --skip-inner "$skip_inner"} \
  ${sweep_loop:+ --sweep-loop "$sweep_loop"} \
