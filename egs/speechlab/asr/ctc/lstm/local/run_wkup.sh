...
dict=/aifs/users/wd007/asr/work2019/aicar/lstm/s1/data/lms/kws.dict
keywords=/aifs/users/wd007/asr/work2019/aicar/lstm/s1/data/lms/raw_keywords3.txt
phonelist=/aifs/users/wd007/asr/work2019/aicar/lstm/s1/data/lms/phonelist.txt
...
#false && \
{
awk 'BEGIN{print "<eps> 0";}{print $1, NR;}' $phonelist > phones.txt
awk -v filler=$filler '{print filler, $1;}' $phonelist > fillerlist.txt

cat $dict fillerlist.txt  | grep -v "<s>" | grep -v "</s>" | grep -v "sil" | LANG= LC_ALL= sort > lexicon.txt  || exit 1;
...
awk -v filler=$filler -v kweight=$kweight -v fweight=$fweight 'BEGIN{e="<eps>";print 0,1,e,e;print 1;c=2}{print 0,c,e,e;for (i=1;i<=NF;i++){print c,c+1,$i,$i,kweight;c++;}print c,0,e,e,0;c++;}END{print 0,c,filler,filler,fweight;print c,0,e,e}' $keywords > G.fst.txt

fstcompile --isymbols=words.txt --osymbols=words.txt G.fst.txt G.fst
echo "Generate G.fst sucessful"
