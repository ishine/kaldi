...
steps/align_ctc_htk.sh --nj 10 --cmd "$train_cmd" \
   $traindir/split3/1 data/lang $srcdir ${srcdir}_ali_train/1 || exit 1;
}

false && \
{
steps/make_denlats_ctc_htk.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_ctc.config --acwt $acwt \
   $traindir/split3/1 data/lang $srcdir ${srcdir}_denlats_train/1 || exit 1;
}

cuda_cmd=run.pl
traindir=data/train_select/v3
dir=${srcdir}_mmi_v3
