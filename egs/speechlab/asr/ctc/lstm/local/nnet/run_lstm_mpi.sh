...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train_mpi.sh --network-type lstm --learn-rate 0.00005  --nnet-init $dir/nnet.init \
      --splice-left 5 --splice-right 5 --online true --cmvn-opts "--cmn-window=10000 --min-cmn-window=100 --norm-vars=false" \
      --train-opts "--momentum 0.9 --halving-factor 0.5" --skip-frames 3 --skip-inner true --sweep-loop false \
      --train-tool "mpiexec -n 8 -f hostfile -wdir . nnet-train-lstm-streams-parallel-mpi --global-momentum=0.90 --num-stream=64 --batch-size=20 --targets-delay=5" \
    ${train}_tr90 ${train}_cv10 data/lang $ali $ali $dir || exit 1;

  # --cmvn-opts "--norm-means=true --norm-vars=true"
  # --nnet-init $nnet --splice 0 
