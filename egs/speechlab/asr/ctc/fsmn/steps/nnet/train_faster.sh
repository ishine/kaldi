...
copy_feats_compress=false # compress feats while resaving
feats_std=1.0
...
if [ -z "$labels" ]; then
  ...
  for f in $alidir/final.mdl $alidir/ali.scp; do
...
if [ ! -z "$labels" ]; then
  ...
  labels_tr="ark:ali-to-pdf $alidir/final.mdl scp:$alidir/ali.tr.scp ark:- | ali-to-post ark:- ark:- |"
  labels_cv="ark:ali-to-pdf $alidir/final.mdl scp:$alidir/ali.cv.scp ark:- | ali-to-post ark:- ark:- |"
