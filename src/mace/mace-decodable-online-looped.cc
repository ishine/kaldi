...
void MaceDecodableNnetLoopedOnlineBase::AdvanceChunk() {
  ...
  int32 sf = info_.opts.frame_subsampling_factor;
  int32 chunk_size = info_.frames_left_context * sf +
      info_.frames_per_chunk + info_.frames_right_context * sf;
  if (num_chunks_computed_ == 0) {
    ...
    // induction.
    begin_input_frame = num_chunks_computed_ * info_.frames_per_chunk - info_.frames_left_context;
  ...
  { // this block sets 'feats_chunk'.
    Matrix<BaseFloat> this_feats(subsampled_chunk_size,
                                 input_features_->Dim());
    for (int32 i = begin_input_frame, j = 0;
         i < end_input_frame;
         i+= sf, j++) {
      SubVector<BaseFloat> this_row(this_feats, j);
      int32 input_frame = i;
