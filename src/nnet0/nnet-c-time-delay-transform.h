...
class CompressedTimeDelayTransform : public UpdatableComponent {
 ...
 public:
  ...
  void InitData(std::istream &is) {
	...
	linearity_u_.Resize(dim_out_, rank_);
	linearity_v_.Resize(rank_, dim_in_*num_indexes_);
	if (param_range == 0.0) {
		RandGauss(0.0, param_stddev, &linearity_u_);
		RandGauss(0.0, param_stddev, &linearity_v_);
	} else {
		RandUniform(0.0, param_range, &linearity_u_);
		RandUniform(0.0, param_range, &linearity_v_);
	}
	// Uniform,
