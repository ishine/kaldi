...
BatchNormTransform(int32 dim_in, int32 dim_out)
  :UpdatableComponent(dim_in, dim_out), sigma_(1e-9), scale_(dim_out), scale_updata_(dim_out),
   shift_(dim_out), shift_updata_(dim_out),mini_batch_mean_(dim_out),
...
void SetParms(Vector<BaseFloat> &scale, Vector<BaseFloat> &shift) {
	scale_ = scale;
	shift_ = shift;
}
