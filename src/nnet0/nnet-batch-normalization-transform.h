// nnet/nnet-batch-normalization-transform.h
...
#ifndef KALDI_NNET0_NNET_BATCH_NORMALIZATION_TRANSFORM_H_
#define KALDI_NNET0_NNET_BATCH_NORMALIZATION_TRANSFORM_H_
...
public:
	...
	void UpdateGradient(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    if (CuDevice::Instantiate().Enabled()) {
  		...
  		size = scale_.Dim()*sizeof(BaseFloat);
  		dst = (void*) (direction==0 ? ((char *)host+pos) : (char *)scale_.Data());
  		src = (void*) (direction==0 ? (char *)scale_.Data() : ((char *)host+pos));
  		cudaMemcpy(dst, src, size, kind);
  		pos += size;

  		size = scale_.Dim()*sizeof(BaseFloat);
  		dst = (void*) (direction==0 ? ((char *)host+pos) : (char *)shift_.Data());
  		src = (void*) (direction==0 ? (char *)shift_.Data() : ((char *)host+pos));
  		cudaMemcpy(dst, src, size, kind);
  		pos += size;
