...
void CuMatrixBase<Real>::AddMatMat(
  ...
  if (CuDevice::Instantiate().Enabled()) {
    CuTimer tim;
    CUBLAS_SAFE_CALL(cublas_gemm(GetLocalCublasHandle(),
...
void CuMatrixBase<Real>::AddVecVec(
  ...
  if (CuDevice::Instantiate().Enabled()) {
    CuTimer tim;
    CUBLAS_SAFE_CALL(cublas_ger(GetLocalCublasHandle(), m, n, alpha,
...
void CuMatrixBase<Real>::SymAddMat2(
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CUBLAS_SAFE_CALL(cublas_syrk(GetLocalCublasHandle(), CUBLAS_FILL_MODE_UPPER,
...
void AddMatMatBatched(const Real alpha, std::vector<CuSubMatrix<Real>* > &C,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CUBLAS_SAFE_CALL(cublas_gemmBatched(C[0]->GetLocalCublasHandle(), //GetCublasHandle(),
                                        (transB==kTrans? CUBLAS_OP_T:CUBLAS_OP_N),
                                        (transA==kTrans? CUBLAS_OP_T:CUBLAS_OP_N),
...
void CopyFromMatStreamed(const std::vector<CuSubMatrix<Real>* > &src,
  ...
  if (CuDevice::Instantiate().Enabled()) {
	  ...
	  CuDevice::Instantiate().AccuProfile("CuMatrixBase::CopyFromMatStreamed(from other CuMatrixBase)", tim);
  } else
#endif
  {
	  for (int32 i = 0; i < size; i++) {
...
template<typename Real>
void CuMatrixBase<Real>::DivRows(const CuMatrixBase<Real> &src,
                                 const CuArrayBase<MatrixIndexT> &indexes) {
  if (NumRows() == 0) return;
  KALDI_ASSERT(static_cast<MatrixIndexT>(indexes.Dim()) == NumRows());
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    ...
    GetBlockSizesForSimpleMatrixOperation(NumRows(), NumCols(),
                                          &dimGrid, &dimBlock);
    cuda_div_rows(dimGrid, dimBlock,
                  data_, src.Data(), indexes.Data(), Dim(), src.Stride());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim);
  } else
