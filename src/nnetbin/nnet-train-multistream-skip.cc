...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // std::string feature_wspecifier = po.GetArg(5);
    ...
    // skip-frame
    std::queue<Matrix<BaseFloat> > q_feats_utt;
    std::queue<Posterior> q_labels_utt;
    std::queue<Vector<BaseFloat> > q_weights_utt;
    ...
    while (1) {
      ...
      for (int s = 0; s < num_streams; s++) {
        ...
        if (feats_utt[s].NumRows() == 0) {
          ...
          Posterior targets;
          Vector<BaseFloat> weights;
          if (!q_feats_utt.empty()) {
            feats_utt[s] = q_feats_utt.front();
            labels_utt[s] = q_labels_utt.front();
            weights_utt[s] = q_weights_utt.front();
            new_utt_flags[s] = 1;
            q_feats_utt.pop();
            q_labels_utt.pop();
            q_weights_utt.pop();
            // feature_writer.Write("PLACEHOLDER", feats_utt[s]);
            continue;
          }
          ...
          if (ReadData(feature_reader, target_reader, weights_reader,
            ...
            // use nnet_transf to do contextual windowing and target delay
            nnet_transf.Feedforward(CuMatrix<BaseFloat>(feats), &feats_transf);

            // skip frames
            Matrix<BaseFloat> feats_transf_tmp(feats_transf);
            int32 utt_frames = feats_transf_tmp.NumRows();
            int32 part_frames = utt_frames/(num_skip_frames+1);
            for (int32 i = 0; i < num_skip_frames + 1; ++i) {
              Matrix<BaseFloat> feats_part;
              Posterior targets_part;
              Vector<BaseFloat> weights_part;

              feats_part.Resize(part_frames, feats_transf_tmp.NumCols(), kSetZero);
              targets_part.resize(part_frames);
              weights_part.Resize(part_frames);
              weights_part.Set(1.0);

              // prepare partial data
              for (int32 f = i, j = 0; f < utt_frames && j < part_frames; f += num_skip_frames+1, ++j) {
                feats_part.Row(j).CopyFromVec(feats_transf_tmp.Row(f));
                targets_part[j] = targets[f];
                weights_part(j) = weights(f);
              }
              // store,
              q_feats_utt.push(feats_part);
              q_labels_utt.push(targets_part);
              q_weights_utt.push(weights_part);
            }
            // fill this stream use queue (use --s to eliminate s++ in for())
            --s;
          }  // if (ReadData)
        }
      }  // for s
