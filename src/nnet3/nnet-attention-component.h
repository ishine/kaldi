 ...
 public:
  ...
  class PrecomputedIndexes: public ComponentPrecomputedIndexes {
   public:
    ...
    virtual void Read(std::istream &os, bool binary);
    virtual std::string Type() const {
      return "SelfAttentionComponentPrecomputedIndexes";
    }
    virtual ~PrecomputedIndexes() { }
  ...
  // This is what's returned as the 'memo' from the Propagate() function.
  struct Memo {
    // c is of dimension (num_heads_ * num-output-frames),
    // where num-output-frames is the number of frames of output the
    // corresponding Propagate function produces.
    // Each block of 'num-output-frames' rows of c_t is the
    // post-softmax matrix of weights.
...
class Contextless3RestrictedAttentionComponent: public Component {
 public:

  // The use of this constructor should only precede InitFromConfig()
  Contextless3RestrictedAttentionComponent() { }

  // Copy constructor
  Contextless3RestrictedAttentionComponent(const Contextless3RestrictedAttentionComponent &other);
  ...
  virtual std::string Info() const;
  virtual void InitFromConfig(ConfigLine *cfl);
  virtual std::string Type() const { return "Contextless3RestrictedAttentionComponent"; }
  virtual int32 Properties() const {
    return kBackpropNeedsInput|kPropagateAdds|kBackpropAdds|
        kUsesMemo;
        //kStoresStats|kUsesMemo;
  }
  virtual void* Propagate(const ComponentPrecomputedIndexes *indexes,
  ...
  */
  ...
  virtual void Write(std::ostream &os, bool binary) const;
  virtual Component* Copy() const {
    return new Contextless3RestrictedAttentionComponent(*this);
  }
  virtual void DeleteMemo(void *memo) const { delete static_cast<Memo*>(memo); }
  ...
  class PrecomputedIndexes: public ComponentPrecomputedIndexes {
   public:
    ...
    virtual void Read(std::istream &os, bool binary);
    virtual std::string Type() const {
      return "Contextless3RestrictedAttentionComponentPrecomputedIndexes";
    }
    virtual ~PrecomputedIndexes() { }
