// latbin/lattice-lmrescore-lstmlm.cc

// Copyright 2015  Guoguo Chen
// Copyright 2017  Wei Deng
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Rescores lattice with lstmlm. The LM will be wrapped into the\n"
    ...
    std::string lats_rspecifier, unk_prob_rspecifier,
        word_symbols_rxfilename, lstmlm_word_symbols_rxfilename,
		lstmlm_rxfilename, lats_wspecifier;
    if (po.NumArgs() == 5) {
      unk_prob_rspecifier = "";
    ...
    KaldiNNlmWrapper lstmlm(opts, unk_prob_rspecifier, word_symbols_rxfilename,
                            lstmlm_word_symbols_rxfilename, lstmlm_rxfilename);
