...
bool DecodeUtterance(LatticeBiglmFasterBucketDecoder &decoder, // not const but is really an input.
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Differ from the standard biglm decoder, this decoder is added the "
        "lazy-evluation bucket technique."
        ...
        "Usage: latgen-biglm-bucket-faster-mapped [options] model-in (fst-in|fsts-rspecifier) "
    ...
    LatticeBiglmFasterBucketDecoderConfig config;
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
      ...
      {
        LatticeBiglmFasterBucketDecoder decoder(*decode_fst, config, &cache_dfst);
        timer.Reset();
