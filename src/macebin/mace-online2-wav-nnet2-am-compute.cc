...
#include "mace/mace-online-nnet2-decoding.h"
#include "online2/online-nnet2-feature-pipeline.h"
#include "online2/onlinebin-util.h"
#include "mace/mace-computer.h"
  ...
  try {
    ...
    for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
      ...
      for (size_t i = 0; i < uttlist.size(); i++) {
        ...
        if (!pad_input)
          output_frames -= computer.LeftContext() + computer.RightContext();
        if (output_frames <= 0) {
          KALDI_WARN << "Skipping utterance " << utt << " because output "
                     << "would be empty.";
          continue;
        }
        computer.AcceptInput("input", &feats_cu);
        computer.Run();
