...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        ...
        "Usage: mace-latgen-faster [options] <trans-model> <model-file> <weight-file>"
        " <config-file> <fst-in|fsts-rspecifier> <features-rspecifier>"
        " <lattice-wspecifier> [ <words-wspecifier> [<alignments-wspecifier>] ]\n"
        "See also: mace-latgen-faster-parallel, mace-latgen-faster-batch\n";
    ParseOptions po(usage);
    Timer timer;
    bool allow_partial = false;
    LatticeFasterDecoderConfig config;
    MaceSimpleComputationOptions decodable_opts;
    ...
    std::string trans_model_file = po.GetArg(1),
//        model_file = po.GetArg(2),
//        weight_file = po.GetArg(3),
//        config_file = po.GetArg(2),
    ...
    {
      bool binary;
      Input ki(trans_model_file, &binary);
    ...
    const std::string model_file = "/home/liutuo/workspace/AI/liutuo/mace/build/cvte/model/cvte.pb";
    const std::string weight_file = "/home/liutuo/workspace/AI/liutuo/mace/build/cvte/model/cvte.data";

    const std::vector<std::string> input_nodes = {"input"};
    const std::vector<std::string> output_nodes = {"output.affine"};
    const std::vector<std::vector<int64_t>> input_shapes = {{1, 72, 40}};
    const std::vector<std::vector<int64_t>> output_shapes = {{1, 50, 6508}};

    MaceComputer computer(model_file,
                          weight_file,
                          input_nodes, // input names
                          output_nodes, // output names
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          DecodableAmMaceSimple nnet_decodable(
              decodable_opts, trans_model, &computer, priors,
